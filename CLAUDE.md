...
`@docs/harness_workflow.md`의 실행 흐름을 **필수**로 진행한다
...
- **중대형만**: Planner를 호출하여 `docs/exec-plans/<type>-<task-name>/SPEC.md`를 생성한다. 호출 시 사용자 요청과 핵심 의도(방향·완료 기준)를 함께 전달한다.
- 테스트 통과 및 Evaluator 합격 후 커밋한다. 상세 절차는 `@docs/harness_workflow.md` 참고.
